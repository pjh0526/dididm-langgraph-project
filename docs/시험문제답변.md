...
**Bug 1**: MemorySaver import 누락

**Bug 2**: @tool 데코레이터를 인스턴스 메서드에 적용
...
**문제점**: checkpointer와 agent를 매 요청마다 새로 생성하고 있음. 
...
**(3)** 문제점: from langchain_core.messages import HumanMessage -> 
        올바른 경로: from langchain.messages import HumanMessage
...
```
ToolStrategy를 활용하면 공식 API 관점에서 안정적인 구조화된 출력(Structured Output)형식을 구현할 수 있으며, 이는 도구 호출(Tool Calling)을 지원하는 모든 모델에서 보편적으로 작동합니다.

반면, ChatResponse를 @tool은 답변을 구조화(response_format)하는 도구를 사용하지 않고 모델이 응답하는 형태의 Schema형태를 그대로 출력하는 형태로 에이전트가 실행 과정중에 특정 데이터를 외부 DB에 저장하거나 특정 포맷으로 동작을 요할때 사용 됩니다.

따라서, 사용자에게 전달할 최종 응답의 규격을 맞추는 것이 목적이라면, response_format=ToolStrategy(ChatResponse)를 통해 스키마 정밀도를 높이고 모델이 도구 호출과 최종 응답을 혼동하지 않게 정교하게 설계할 수 있습니다.
```
